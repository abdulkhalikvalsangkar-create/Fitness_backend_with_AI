...
def handle_chat(body: dict, principal: Principal, session: Session) -> dict[str, Any]:
    ...
    for block in payload.blocks:
        if not block.data:
            continue
        for raw_job_id in block.data.get("job_ids") or []:
            job_id = str(raw_job_id)
            if job_id not in seen_pending:
                seen_pending.add(job_id)
...
def handle_biological_age_calculator(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail=f"biological age module unavailable: {type(exc).__name__}: {exc}",
            ) from exc

        # --- 2. validate structured params ----------------------------------
        stage = "validate_age"
        try:
        ...
        tb_lines = traceback.format_exception(type(exc), exc, exc.__traceback__)
        tb_str = "".join(tb_lines[-8:])
        logger.exception(
            "BIOAGE[%s] stage=%s TOPLEVEL %s: %s\n%s",
            principal.user_id, stage, type(exc).__name__, exc, tb_str,
        )
        raise HTTPException(
            status_code=500,
